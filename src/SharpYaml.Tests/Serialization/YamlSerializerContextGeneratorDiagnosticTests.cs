...
public class YamlSerializerContextGeneratorDiagnosticTests
{
    ...
    [TestMethod]
    public void GeneratorEmitsGeneratedFileHeaderAndTypeAttributes()
    {
        const string source = """
            using SharpYaml.Serialization;
            ...
            [YamlSerializable(typeof(MetadataPayload))]
            internal partial class MetadataYamlContext : YamlSerializerContext
            {
            }
            """;

        var result = RunGenerator(source);
        var generatedCodeAttribute = "[global::System.CodeDom.Compiler.GeneratedCode(@\"SharpYaml.SourceGenerator\", @\"" +
            (typeof(YamlSerializerContextGenerator).Assembly.GetName().Version?.ToString() ?? "0.0.0.0") +
            "\")]";
        var excludeFromCoverageAttribute = "[global::System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage]";

        Assert.IsTrue(result.GeneratedSource.StartsWith("// <auto-generated/>", StringComparison.Ordinal), result.GeneratedSource);

        var pragmaIndex = result.GeneratedSource.IndexOf("#pragma warning disable", StringComparison.Ordinal);
        var nullableIndex = result.GeneratedSource.IndexOf("#nullable enable annotations", StringComparison.Ordinal);
        Assert.IsTrue(pragmaIndex >= 0, result.GeneratedSource);
        Assert.IsTrue(nullableIndex > pragmaIndex, result.GeneratedSource);

        StringAssert.Contains(
            result.GeneratedSource,
            generatedCodeAttribute + Environment.NewLine +
            excludeFromCoverageAttribute + Environment.NewLine +
            "partial class MetadataYamlContext");
        StringAssert.Contains(
            result.GeneratedSource,
            "    " + generatedCodeAttribute + Environment.NewLine +
            "    " + excludeFromCoverageAttribute + Environment.NewLine +
            "    private sealed class RuntimeCustomConverterCache");
        StringAssert.Contains(
            result.GeneratedSource,
            "    " + generatedCodeAttribute + Environment.NewLine +
            "    " + excludeFromCoverageAttribute + Environment.NewLine +
            "    private sealed class GeneratedTypeInfo0");
    }
