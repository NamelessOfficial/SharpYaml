...
public sealed class YamlSerializerContextGenerator : IIncrementalGenerator
{
    ...
    {
        builder.AppendLine("    private static bool TryGetRuntimeCustomConverter(global::SharpYaml.Serialization.YamlReaderWriterBase readerWriter, global::System.Type typeToConvert, out global::SharpYaml.Serialization.YamlConverter? converter)");
        builder.AppendLine("    {");
            ...
            if (named.TypeKind == TypeKind.Class && TryGetPolymorphismInfo(named, derivedTypeMappings, out var polymorphism) && polymorphism.DerivedTypes.Length != 0)
            {
                ...
                for (var i = 0; i < polymorphism.DerivedTypes.Length; i++)
                {
                    ...
                    builder.Append("            WriteMembers").Append(derivedIndex).Append("(writer, ").Append(derivedLocal).Append(", ")
                        .Append(mayWriteDiscriminatorProperty ? "discriminatorPropertyName" : "null").AppendLine(");");
                    builder.AppendLine("            writer.WriteEndMapping();");
                    if (emitLifecycleCallbacks)
                    {
