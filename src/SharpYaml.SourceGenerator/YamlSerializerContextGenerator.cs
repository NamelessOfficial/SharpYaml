...
{
    private const string GeneratedCodeTool = "SharpYaml.SourceGenerator";
        ...
        Dictionary<ITypeSymbol, int> indexByType)
    {
        ...
        for (var index = 0; index < types.Length; index++)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("        public GeneratedTypeInfo" + index + "(global::SharpYaml.YamlSerializerOptions options, RuntimeCustomConverterCache runtimeConverters) : base(options)");
            builder.AppendLine("        {");
        ...
        for (var index = 0; index < types.Length; index++)
        {
            ...
            EmitWriteValue(builder, index, types[index], typeName, derivedTypeMappings, indexByType, propertyNamingPolicy, model.SourceGenerationOptions);
            builder.AppendLine();
            EmitReadValue(builder, index, types[index], typeName, derivedTypeMappings, indexByType, propertyNamingPolicy, model.SourceGenerationOptions);
            builder.AppendLine();
        }
    ...
    private static void EmitSourceGenerationConverterFields(StringBuilder builder, SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        {
            var converterTypeName = sourceGenerationOptions.ConverterTypes[i].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
    ...
    private static string GetSourceGenerationConverterFieldName(int converterIndex)
        => "s_sourceGenerationConverter" + converterIndex.ToString(global::System.Globalization.CultureInfo.InvariantCulture);
    ...
    private static void EmitRuntimeCustomConverterHelpers(StringBuilder builder, SourceGenerationOptionsModel sourceGenerationOptions, ImmutableArray<ITypeSymbol> runtimeCustomConverterTypes)
    {
        ...
        builder.AppendLine("    {");
        builder.AppendLine("        private readonly global::System.Collections.Generic.Dictionary<global::System.Type, global::SharpYaml.Serialization.YamlConverter>? _converters;");
        builder.AppendLine();
        builder.AppendLine("        public RuntimeCustomConverterCache(global::SharpYaml.YamlSerializerOptions options)");
        builder.AppendLine("        {");
        ...
        builder.AppendLine("                {");
        builder.AppendLine("                    _converters ??= new global::System.Collections.Generic.Dictionary<global::System.Type, global::SharpYaml.Serialization.YamlConverter>();");
        builder.AppendLine("                    _converters[typeToConvert] = converter;");
        builder.AppendLine("                }");
        builder.AppendLine("            }");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        public bool TryGetConverter(global::System.Type typeToConvert, out global::SharpYaml.Serialization.YamlConverter? converter)");
        builder.AppendLine("        {");
        ...
        builder.AppendLine();
        builder.AppendLine("    private static bool IsSourceGenerationConverter(global::SharpYaml.Serialization.YamlConverter converter)");
        builder.AppendLine("        => ");
        for (var i = 0; i < sourceGenerationOptions.ConverterTypes.Length; i++)
        {
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        builder.Append("    private static void WriteValue").Append(index)
            .Append("(global::SharpYaml.Serialization.YamlWriter writer, ").Append(typeName).AppendLine(" value, RuntimeCustomConverterCache runtimeConverters)");
        builder.AppendLine("    {");
        ...
        if (typeSymbol is INamedTypeSymbol named && (named.TypeKind == TypeKind.Class || named.TypeKind == TypeKind.Struct))
        {
            ...
            var members = GetSerializableMembers(named)
                ...
                .ToImmutableArray();
            builder.Append("        WriteMembers").Append(index).AppendLine("(writer, value, null, runtimeConverters);");
            builder.AppendLine("        writer.WriteEndMapping();");
            if (emitLifecycleCallbacks)
            {
                builder.AppendLine("        InvokeOnSerialized();");
    ...
    private static void EmitWriteExtensionDataDictionaryEntries(StringBuilder builder, string valueTypeName, bool? sortedMappingOrder)
    {
        EmitMappingOrderBranch(
            ...
            sortedIndent =>
            {
                ...
                EmitWriteExtensionDataEntryKeyGuard(builder, sortedIndent + "    ");
                ...
                EmitWriteExtensionDataValue(builder, valueTypeName, sortedIndent + "    ");
                builder.Append(sortedIndent).AppendLine("}");
            },
            declarationIndent =>
            {
                ...
                EmitWriteExtensionDataEntryKeyGuard(builder, declarationIndent + "    ");
                ...
                EmitWriteExtensionDataValue(builder, valueTypeName, declarationIndent + "    ");
                builder.Append(declarationIndent).AppendLine("}");
            });
    }
    ...
    private static void EmitWriteExtensionDataMappingEntries(StringBuilder builder, bool? sortedMappingOrder)
    {
        ...
        EmitMappingOrderBranch(
            ...
            sortedIndent =>
            {
                builder.Append(sortedIndent).AppendLine("var items = new global::System.Collections.Generic.List<global::System.Collections.Generic.KeyValuePair<string, global::SharpYaml.Model.YamlElement?>>(list.Count);");
                builder.Append(sortedIndent).AppendLine("for (var i = 0; i < list.Count; i++)");
                builder.Append(sortedIndent).AppendLine("{");
                builder.Append(sortedIndent).AppendLine("    var pair = list[i];");
                builder.Append(sortedIndent).AppendLine("    if (pair.Key is not global::SharpYaml.Model.YamlValue keyValue)");
                builder.Append(sortedIndent).AppendLine("    {");
                builder.Append(sortedIndent).AppendLine("        throw new global::SharpYaml.YamlException(global::SharpYaml.Mark.Empty, global::SharpYaml.Mark.Empty, \"Only scalar mapping keys are supported for extension data.\");");
                builder.Append(sortedIndent).AppendLine("    }");
                builder.Append(sortedIndent).AppendLine("    items.Add(new global::System.Collections.Generic.KeyValuePair<string, global::SharpYaml.Model.YamlElement?>(keyValue.Value, pair.Value));");
                builder.Append(sortedIndent).AppendLine("}");
                builder.Append(sortedIndent).AppendLine("items.Sort(static (x, y) => global::System.String.CompareOrdinal(x.Key, y.Key));");
                builder.Append(sortedIndent).AppendLine("for (var i = 0; i < items.Count; i++)");
                builder.Append(sortedIndent).AppendLine("{");
                ...
                builder.Append(sortedIndent).AppendLine("    writer.WritePropertyName(key);");
                builder.Append(sortedIndent).AppendLine("    var extensionValue = items[i].Value;");
                EmitWriteExtensionDataYamlNodeValue(builder, sortedIndent + "    ");
                builder.Append(sortedIndent).AppendLine("}");
            },
            declarationIndent =>
            {
                ...
                builder.Append(declarationIndent).AppendLine("    if (pair.Key is not global::SharpYaml.Model.YamlValue keyValue)");
                builder.Append(declarationIndent).AppendLine("    {");
                builder.Append(declarationIndent).AppendLine("        throw new global::SharpYaml.YamlException(global::SharpYaml.Mark.Empty, global::SharpYaml.Mark.Empty, \"Only scalar mapping keys are supported for extension data.\");");
                builder.Append(declarationIndent).AppendLine("    }");
                builder.Append(declarationIndent).AppendLine("    var key = keyValue.Value;");
                EmitWriteExtensionDataEntryKeyGuard(builder, declarationIndent + "    ");
                builder.Append(declarationIndent).AppendLine("    writer.WritePropertyName(key);");
                builder.Append(declarationIndent).AppendLine("    var extensionValue = pair.Value;");
                EmitWriteExtensionDataYamlNodeValue(builder, declarationIndent + "    ");
                builder.Append(declarationIndent).AppendLine("}");
            });
    }
    ...
    private static void EmitMappingOrderBranch(StringBuilder builder, bool? sortedMappingOrder, string indent, Action<string> emitSorted, Action<string> emitDeclaration)
    {
        ...
        builder.Append(indent).AppendLine("if (options.MappingOrder == global::SharpYaml.YamlMappingOrderPolicy.Sorted)");
    ...
    private static void EmitWriteExtensionDataEntryKeyGuard(StringBuilder builder, string indent)
    {
        builder.Append(indent).AppendLine("if (discriminatorPropertyName is not null && global::System.String.Equals(key, discriminatorPropertyName, global::System.StringComparison.Ordinal))");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).AppendLine("    continue;");
        builder.Append(indent).AppendLine("}");
    }

    private static void EmitWriteExtensionDataValue(StringBuilder builder, string valueTypeName, string indent)
    {
        builder.Append(indent).AppendLine("if (extensionValue is null)");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).AppendLine("    writer.WriteNullValue();");
        builder.Append(indent).AppendLine("}");
        builder.Append(indent).AppendLine("else");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).Append("    var converter = writer.GetConverter(typeof(").Append(valueTypeName).AppendLine("));");
        builder.Append(indent).AppendLine("    converter.Write(writer, extensionValue);");
        builder.Append(indent).AppendLine("}");
    }

    private static void EmitWriteExtensionDataYamlNodeValue(StringBuilder builder, string indent)
    {
        builder.Append(indent).AppendLine("if (extensionValue is null)");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).AppendLine("    writer.WriteNullValue();");
        ...
        builder.Append(indent).AppendLine("else");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).AppendLine("    var converter = writer.GetConverter(typeof(global::SharpYaml.Model.YamlNode));");
        builder.Append(indent).AppendLine("    converter.Write(writer, extensionValue);");
        builder.Append(indent).AppendLine("}");
    }
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        builder.AppendLine("        if (options.DefaultIgnoreCondition is global::SharpYaml.YamlIgnoreCondition.Always or global::SharpYaml.YamlIgnoreCondition.WhenWriting)");
        builder.AppendLine("        {");
        builder.AppendLine("            // Skip value ignored during serialization.");
        builder.AppendLine("        }");
        builder.AppendLine("        else if (options.DefaultIgnoreCondition == global::SharpYaml.YamlIgnoreCondition.WhenWritingNull)");
        builder.AppendLine("        {");
        EmitWriteMemberWithNullCheck(builder, member, indexByType, memberValueVar, nameVar, indent: "            ", sourceGenerationOptions);
        builder.AppendLine("        }");
        builder.AppendLine("        else if (options.DefaultIgnoreCondition == global::SharpYaml.YamlIgnoreCondition.WhenWritingDefault)");
        builder.AppendLine("        {");
        EmitWriteMemberWithDefaultCheck(builder, member, indexByType, memberValueVar, nameVar, indent: "            ", sourceGenerationOptions);
        builder.AppendLine("        }");
    ...
    private static void EmitWriteMemberWithKnownIgnoreCondition(
        StringBuilder builder,
        MemberModel member,
        Dictionary<ITypeSymbol, int> indexByType,
        string memberValueVar,
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        {
            case IgnoreAlways:
            case IgnoreWhenWriting:
                builder.AppendLine("        // Skip value ignored during serialization.");
                return;

            case IgnoreWhenWritingNull:
                builder.Append("        var ").Append(memberValueVar).Append(" = ").Append(member.AccessExpression).AppendLine(";");
                EmitWriteMemberWithNullCheck(builder, member, indexByType, memberValueVar, nameVar, indent: "        ", sourceGenerationOptions);
                return;

            case IgnoreWhenWritingDefault:
                builder.Append("        var ").Append(memberValueVar).Append(" = ").Append(member.AccessExpression).AppendLine(";");
                EmitWriteMemberWithDefaultCheck(builder, member, indexByType, memberValueVar, nameVar, indent: "        ", sourceGenerationOptions);
                return;
    ...
    private static void EmitWriteMemberWithNullCheck(
        StringBuilder builder,
        MemberModel member,
        Dictionary<ITypeSymbol, int> indexByType,
        string memberValueVar,
    ...
    private static void EmitWriteMemberWithDefaultCheck(
        StringBuilder builder,
        MemberModel member,
        Dictionary<ITypeSymbol, int> indexByType,
        string memberValueVar,
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        builder.Append(indent).Append("if (global::System.Collections.Generic.EqualityComparer<").Append(member.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)).Append(">.Default.Equals(")
    ...
    private static void EmitWriteMemberProperty(
        StringBuilder builder,
        MemberModel member,
        Dictionary<ITypeSymbol, int> indexByType,
        string memberValueVar,
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        EmitWriteMemberValueWithCustomConverter(builder, member, indexByType, valueExpression: memberValueVar, indent: indent + "    ", sourceGenerationOptions);
        builder.Append(indent).AppendLine("}");
    }

    private static void EmitReadObjectCoreMethod(StringBuilder builder, int index, ITypeSymbol typeSymbol, string typeName, ImmutableArray<MemberModel> members, ExtensionDataMemberModel? extensionData, Dictionary<ITypeSymbol, int> indexByType, JsonNamingPolicy? propertyNamingPolicy, SourceGenerationOptionsModel sourceGenerationOptions)
    {
        var emitLifecycleCallbacks =
        ...
        foreach (var member in readCandidates)
        {
            builder.Append("                if (!matched && global::System.String.Equals(mergeKey, ").Append(member.SerializedNameExpressionForRead)
                .Append(", ").Append(propertyNameComparisonExpression).Append("))");
            builder.AppendLine();
            builder.AppendLine("                {");
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        if (TryEmitReadWithStaticOptionsConverter(builder, sourceGenerationOptions, typeSymbol, "        ", convertedValue =>
        {
            builder.Append("            return ").Append(convertedValue).AppendLine(";");
        }))
        {
        ...
        if (typeSymbol is INamedTypeSymbol named && (named.TypeKind == TypeKind.Class || named.TypeKind == TypeKind.Struct))
        {
            ...
            if (named.TypeKind == TypeKind.Class && TryGetPolymorphismInfo(named, derivedTypeMappings, out var polymorphism) && polymorphism.DerivedTypes.Length != 0)
            {
                ...
                builder.AppendLine("    }");
                builder.AppendLine();
                EmitReadObjectCoreMethod(builder, index, typeSymbol, typeName, members, extensionData, indexByType, propertyNamingPolicy, sourceGenerationOptions);
                return;
            }
    ...
    private static void EmitReadMemberValue(StringBuilder builder, MemberModel member, Dictionary<ITypeSymbol, int> indexByType, SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        if (TryGetDictionaryTypes(member.Type, out var dictionaryKeyType, out var dictionaryValueType, out _))
        {
            ...
            if (dictionaryKeyType.SpecialType == SpecialType.System_String)
            {
                builder.Append("                    var dictionary = new ").Append(memberDictionaryTypePrefix).Append("<string, ").Append(valueTypeName)
                    .Append(">(").Append(propertyNameComparerExpression).AppendLine(");");
                builder.Append("                    var dictionaryMergeEnabled = ").Append(mergeEnabledExpression).AppendLine(";");
        ...
        if (indexByType.TryGetValue(member.Type, out var typeIndex))
        {
            builder.Append("                ").Append(member.AssignExpression($"ReadValue{typeIndex}(reader, runtimeConverters)")).AppendLine(";");
            return;
        }
        ...
        SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        if (explicitPopulate)
        {
            EmitPopulateObjectMemberValue(builder, member, memberTypeName, declaringTypeName, indexByType, readOnlyFallbackStatement, canAssign, canPopulateValueType, explicitPopulate: true, indent: "                ", sourceGenerationOptions);
            return;
        }
        ...
        builder.AppendLine("                if (options.PreferredObjectCreationHandling == global::System.Text.Json.Serialization.JsonObjectCreationHandling.Populate)");
        builder.AppendLine("                {");
        EmitPopulateObjectMemberValue(builder, member, memberTypeName, declaringTypeName, indexByType, readOnlyFallbackStatement, canAssign, canPopulateValueType, explicitPopulate: false, indent: "                    ", sourceGenerationOptions);
        builder.AppendLine("                }");
        builder.AppendLine("                else");
    ...
    private static void EmitReadMemberValueWithCustomConverter(StringBuilder builder, MemberModel member, Dictionary<ITypeSymbol, int> indexByType, SourceGenerationOptionsModel sourceGenerationOptions)
    {
        ...
        {
            builder.Append("                        ").Append(member.AssignExpression(convertedValue)).AppendLine(";");
        }))
        {
            builder.AppendLine("                }");
            builder.AppendLine("                }");
    ...
    private static bool TryEmitWriteWithStaticOptionsConverter(
        StringBuilder builder,
        SourceGenerationOptionsModel sourceGenerationOptions,
        ITypeSymbol typeSymbol,
        ...
        bool emitReturn)
    {
        if (!TryGetStaticOptionsConverterType(sourceGenerationOptions, typeSymbol, out _, out var converterIndex))
        {
            return false;
        }

        builder.Append(indent)
            .Append(GetSourceGenerationConverterFieldName(converterIndex))
            .Append(".Write(writer, ")
    ...
    private static bool TryEmitReadWithStaticOptionsConverter(
        StringBuilder builder,
        SourceGenerationOptionsModel sourceGenerationOptions,
        ITypeSymbol typeSymbol,
        string indent,
        Action<string> emitAssignment)
    {
        ...
        emitAssignment(GetSourceGenerationConverterFieldName(converterIndex) + ".Read(reader)");
        return true;
    }
    ...
    private static void EmitReadKnownType(StringBuilder builder, SourceGenerationOptionsModel sourceGenerationOptions, ITypeSymbol typeSymbol, Dictionary<ITypeSymbol, int> indexByType, string valueVarName, string indent)
    {
        ...
        builder.Append(innerIndent).AppendLine("{");
        var bodyIndent = innerIndent + "    ";
        if (TryEmitReadWithStaticOptionsConverter(builder, sourceGenerationOptions, typeSymbol, bodyIndent, convertedValue =>
        {
            builder.Append(bodyIndent).Append(valueVarName).Append(" = ").Append(convertedValue).AppendLine(";");
        }))
        {
            builder.Append(innerIndent).AppendLine("}");
            builder.Append(indent).AppendLine("}");
        ...
        if (IsKnownScalar(typeSymbol))
        {
            ...
            builder.Append(bodyIndent).AppendLine("if (reader.TokenType != global::SharpYaml.Serialization.YamlTokenType.Scalar)");
            builder.Append(bodyIndent).AppendLine("{");
            builder.Append(bodyIndent).AppendLine("    throw global::SharpYaml.Serialization.YamlThrowHelper.ThrowExpectedScalar(reader);");
            builder.Append(bodyIndent).AppendLine("}");
            if (acceptsNull)
            {
                builder.Append(bodyIndent).AppendLine("if (global::SharpYaml.Serialization.YamlScalar.IsNull(reader))");
                builder.Append(bodyIndent).AppendLine("{");
                builder.Append(bodyIndent).AppendLine("    reader.Read();");
                builder.Append(bodyIndent).AppendLine("}");
                builder.Append(bodyIndent).AppendLine("else");
                builder.Append(bodyIndent).AppendLine("{");
                EmitReadScalarAssignment(builder, scalarType, valueVarName, indent: bodyIndent + "    ");
                builder.Append(bodyIndent).AppendLine("    reader.Read();");
                builder.Append(bodyIndent).AppendLine("}");
            }
            else
        ...
        builder.Append(bodyIndent).AppendLine("throw global::SharpYaml.Serialization.YamlThrowHelper.ThrowNotSupported(reader, \"The generated YAML serializer does not support this element type.\");");
        builder.Append(innerIndent).AppendLine("}");
        builder.Append(indent).AppendLine("}");
    }
    ...
    private static MemberModel CreateMemberModel(ISymbol member, JsonNamingPolicy? propertyNamingPolicy)
    {
        ...
        var ignoreCondition = TryGetIgnoreCondition(member, out var condition) ? condition : (int?)null;
        ...
        var isIgnoredOnRead = ignoreCondition == IgnoreWhenReading;
        var isInitOnly = member is IPropertySymbol property && IsInitOnlyProperty(property);
        return new MemberModel(member, type, nameForRead, nameForWrite, accessExpression, assign, ignoreCondition, converterTypeName, objectCreationHandling, blockSequenceMappingStyle, blockSequenceSequenceStyle, isRequired, isIgnoredOnRead, isInitOnly, isRequiredKeyword);
    }
    ...
    private static string GetMergeEnabledExpression(SourceGenerationOptionsModel options)
        => options.Schema switch
        {
            ...
            _ => "options.Schema is global::SharpYaml.YamlSchemaKind.Core or global::SharpYaml.YamlSchemaKind.Extended",
        };
        ...
        string indent)
    {
        ...
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).AppendLine("    case global::SharpYaml.YamlDuplicateKeyHandling.Error:");
        builder.Append(indent).Append("        throw global::SharpYaml.Serialization.YamlThrowHelper.ThrowDuplicateMappingKey(reader, ").Append(duplicateKeyExpression).AppendLine(");");
        builder.Append(indent).AppendLine("    case global::SharpYaml.YamlDuplicateKeyHandling.FirstWins:");
        builder.Append(indent).AppendLine("        break;");
        builder.Append(indent).AppendLine("    case global::SharpYaml.YamlDuplicateKeyHandling.LastWins:");
        ...
        string indent)
    {
        ...
        builder.Append(indent).AppendLine("if (unknownDerivedTypeHandling == global::SharpYaml.YamlUnknownDerivedTypeHandling.Fail)");
        builder.Append(indent).AppendLine("{");
        builder.Append(indent).Append("    throw ").Append(throwExpression).AppendLine(";");
        builder.Append(indent).AppendLine("}");
    }
    ...
    private static bool TryGetStaticOptionsConverterType(SourceGenerationOptionsModel options, ITypeSymbol type, out ITypeSymbol converterType, out int converterIndex)
    {
        if (!options.ConverterTypes.IsDefaultOrEmpty)
        {
            for (var i = 0; i < options.ConverterTypes.Length; i++)
            {
                var candidate = options.ConverterTypes[i];
                for (var current = candidate as INamedTypeSymbol; current is not null; current = current.BaseType)
                {
                    if (current.IsGenericType &&
                        current.ConstructedFrom.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) == "global::SharpYaml.Serialization.YamlConverter<T>" &&
        ...
        foreach (var attribute in baseType.GetAttributes())
        {
            ...
            if (string.Equals(attributeName, "SharpYaml.Serialization.YamlPolymorphicAttribute", StringComparison.Ordinal))
            {
                foreach (var pair in attribute.NamedArguments)
                {
                    ...
                    {
                        discriminatorStyleOverrideValue = styleValue == -1 ? null : styleValue;
                    }
    ...
    private static string? GetObjectCreationHandling(ISymbol member)
    {
        foreach (var attribute in member.GetAttributes())
        {
            if (!string.Equals(attribute.AttributeClass?.ToDisplayString(), "System.Text.Json.Serialization.JsonObjectCreationHandlingAttribute", StringComparison.Ordinal))
            {
                continue;
            }
    ...
    private static void AppendOptionAssignments(StringBuilder builder, SourceGenerationOptionsModel options)
    {
        ...
        {
            builder.Append("            PreferredObjectCreationHandling = global::System.Text.Json.Serialization.JsonObjectCreationHandling.")
